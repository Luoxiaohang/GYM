 ...
 * type="text" name="date" readOnly onClick="setDay(this);"> (2)选择日期和小时 <input
 * type="text" name="dateh" readOnly onClick="setDayH(this);"> (3)选择日期和小时及分钟
 * <input type="text" name="datehm" readOnly onClick="setDayHM(this);"> 设置参数的方法
 * (1)设置日期分隔符 setDateSplit(strSplit);默认为"-" (2)设置日期与时间之间的分隔符
 * setDateTimeSplit(strSplit);默认为" " (3)设置时间分隔符 setTimeSplit(strSplit);默认为":"
 * (4)设置(1),(2),(3)中的分隔符 setSplit(strDateSplit,strDateTimeSplit,strTimeSplit);
 * (5)设置开始和结束年份 setYearPeriod(intDateBeg,intDateEnd) 说明：
...
// 链接的样式
var s_tiannet_link = "text-decoration:none;font-size:9pt;color:#FFFFFF;";
// 横线
...
document.write('<div id="divTiannetDate" style="' + s_tiannet_body
		+ '" style="本日历选择控件由tiannet根据前人经验完善而成！">');
document
...
document.write('<input type="button" style="' + s_tiannet_turn
		+ '" value="年↑" title="上一年" onClick="tiannetPrevYear();">');
document.write('<input type="button" style="' + s_tiannet_turn
		+ '" value="年↓" title="下一年" onClick="tiannetNextYear();">&nbsp;');
document.write('<input type="button" style="' + s_tiannet_turn
		+ '" value="月↑" title="上一月" onClick="tiannetPrevMonth();">');
document.write('<input type="button" style="' + s_tiannet_turn
		+ '" value="月↓" title="下一月" onClick="tiannetNextMonth();">');
document.write('</div>');
// 输出一条横线
document.write('<div style="' + s_tiannet_line + '"></div>');
document
...
for ( var i = 0; i < weekName.length; i++) {
	// 输出星期
	document.write('<td width="21" align="center" Author="tiannet">'
			+ weekName[i] + '</td>');
}
document.write(' </tr>');
